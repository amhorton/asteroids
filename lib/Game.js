...
Game.prototype.checkShipAsteroidCollisions = function () {
  for(i = 0; i < this.allObjects.length; i++) {
    if (this.allObjects[i] instanceof Asteroid) {
      if(this.allObjects[i].isCollidedWith(this.ship)) {;
...
Game.prototype.checkBulletAsteroidCollisions = function () {
  for (i = 0; i < this.allObjects.length; i++) {
    if (this.allObjects[i] instanceof Bullet) {
      for (j = 0; j < this.allObjects.length; j++) {
        if (this.allObjects[j] instanceof Asteroid) {
